...
- <img src="docs/assets/sprites/chest.png" width="32" height="32" align="absmiddle"> **A campaign OS, not a stack of pages** — running a session needs several tools at once, so the app is a tiling window manager rather than one surface at a time. Split the chat beside the session log and the encounter builder; tab the campaign graph with Ask campaign; drag the gutters, and every arrangement is a **workspace** you switch with `Alt+1..9`. Workspaces are **per game** — Magic and D&D each keep their own set, so changing games swaps the whole desk instead of closing things on your behalf, and switching back is one keystroke. Layouts save to your account, not the browser, so the arrangement you prepped on a laptop is the one waiting on the tablet at the table. The whole thing is keyboard-drivable: `Alt+arrows` move focus, `Alt+Shift+arrows` move windows, and a `Ctrl+G` leader chord reaches every command the browser will not let a web app bind — press `?` for the sheet, which is generated from the keymap so it cannot drift.
...
- <img src="docs/assets/sprites/magnifier.png" width="32" height="32" align="absmiddle"> **Card search, in the sage's hands** — "name every four-colour card", "which cards have pirate ships in the art?", "cards with *sphere* in the name": questions about *which* cards exist are answered by the sage composing a real Scryfall search mid-answer (`c=4`, `art:pirate art:ship`, `name:sphere`) and reading the result — never from memory. The answer carries the match count and a link to the full list on Scryfall, and the search sits under the answer as a citation. Scryfall's own syntax corrections are fed back to the sage, so a malformed query is retried rather than reported as "no such cards". Magic only.
...
- <img src="docs/assets/sprites/swords.png" width="32" height="32" align="absmiddle"> **Encounter builder** — a D&D prep surface you can walk into with no idea. Say how many players and what level, pick a difficulty, and — optionally — type a mood ("something creepy in the swamp"); the builder hands back a whole encounter: a name, a roster, terrain, tactics, a twist and scaling advice. The **server** does the parts that must be right — the 2014 DMG's XP budget, the group shapes that budget can take, and a shortlist of real SRD statblocks inside the affordable challenge-rating window — and the model picks from that shortlist and writes the encounter around it. Every creature it names is resolved back against a **locally mirrored SRD bestiary** before it reaches the page; anything invented is reported, never rendered as real, and XP is always re-derived from the Monster Manual table. Revise it in plain English ("fewer goblins, one nastier thing"), expand any line to its complete statblock in place, and the difficulty gauge recomputes from the finished roster. Monster search is instant and works offline, because the bestiary is already on disk.
- <img src="docs/assets/sprites/hourglass.png" width="32" height="32" align="absmiddle"> **Campaign sessions** — the campaign's memory of what was actually played. Each sitting is a record: raw sources (a pasted transcript, DM notes, a player journal, a chat log, or `.txt`/`.md`/`.srt`/`.vtt` uploads — subtitle files parsed with their timing preserved) stored **verbatim, immutable and checksummed**, plus a chronological log of rulings, Q&A, notes, discoveries and encounters with a one-button in-play `+ Discovery`. Everything downstream cites **addressable spans** — byte offsets into those exact sources plus the quoted text — so "why does Grimoire think this?" resolves to actual words someone actually said; selecting text in a source shows the span. Logging a ruling full-text-matches it against every earlier ruling in the same campaign and surfaces the top hits (*"you ruled the other way on this three sessions ago"* — no LLM involved), and the whole session exports as Markdown.
...
- <img src="docs/assets/sprites/scrollOpen.png" width="32" height="32" align="absmiddle"> **Reading mode** — the corpus is also a shelf of real books. The **Read** button opens a contents column beside a parchment page and the whole rule set browsable as it was written: Magic's Comprehensive Rules as chapters, sections and glossary entries; each SRD document as its own volume; every local D&D book (the PDFs dropped in `DND_DOCS_DIR`) as its own too. Reading pages keep the *raw* markdown — class tables, stat blocks and equipment tables arrive as actual tables — and citations deep-link: "Read this in the book" on any rule in the drawer opens the reader at the page that carries it.
...
| `TRANSCRIBE_MODEL`    | _(empty)_                        | Enables the session audio→transcript hook and the table's push-to-talk server path when set; unset means the affordances are not there. |
...
| `GET`  | `/api/campaigns` | —                                           | `{campaigns:[{id,name,system,premise,role,…}]}` — yours plus any you play in |
| `POST` | `/api/campaigns` | `{name, system?, premise?}`                 | `{campaign:{…}}`; the creator becomes its DM |
| `GET`  | `/api/campaigns/{cid}/sessions` | —                                  | `{sessions:[{id,ordinal,name,status,…}]}` |
| `POST` | `/api/campaigns/{cid}/sessions` | `{name?}` (DM)                      | `{session:{…}}`; ordinal is assigned |
| `PATCH`| `/api/campaigns/{cid}/sessions/{sid}` | `{name?, status?}` (DM)        | `{session:{…}}`; live/done record their own timestamps |
| `GET`  | `/api/campaigns/{cid}/sessions/{sid}/sources` | —                           | `{sources:[…]} — metadata only; `dm_notes`/`live_mark` rows are DM-only, filtered in SQL |
| `POST` | `/api/campaigns/{cid}/sessions/{sid}/sources` | JSON `{kind,author?,title?,content,format?}` or multipart `file` (DM) | `{source:{…}}` — `.txt/.md/.srt/.vtt` parsed by extension; stored verbatim + sha256 |
| `GET`  | `/api/campaigns/{cid}/sessions/{sid}/sources/{srcid}` | —                    | `{source:{content,…}, timing:[…]}` — the verbatim text and cue list |
| `GET`  | `/api/campaigns/{cid}/sessions/{sid}/span` | `source_id` + `start`+`end`, or `quote` | `{span:{source_id,start,end,quote}}` — offsets→quote or quote→offsets |
| `GET`  | `/api/campaigns/{cid}/sessions/{sid}/events` | —                        | `{events:[{seq,kind,summary,detail,payload,…}]}` in play order |
| `POST` | `/api/campaigns/{cid}/sessions/{sid}/events` | `{kind,summary?,detail?,payload?}` (DM) | `{event:{…}, prior_matches:[…]}` — rulings/Q&A FTS-match the campaign's past rulings |
| `GET`  | `/api/campaigns/{cid}/sessions/{sid}/export` | — (DM)                    | Markdown attachment — the whole session log |
