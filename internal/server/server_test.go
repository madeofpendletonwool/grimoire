...
func newTestServer(t *testing.T, scryfallHandler http.HandlerFunc) (*Server, *httptest.Server) {
	...
	s, err := New(store, llm.New(llm.Config{}), cards.NewWithBase(srv.URL), rulings.NewWithBase(srv.URL), nil, nil, Auth{})
	if err != nil {
...
func TestLookupCardRulings_NilService(t *testing.T) {
	...
	t.Cleanup(func() { _ = store.Close() })
	srv, err := New(store, llm.New(llm.Config{}), nil, nil, nil, nil, Auth{})
	if err != nil {
		t.Fatal(err)
	}
