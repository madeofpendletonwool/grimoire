...
| Combat | `DECLARE_ATTACKERS`, `DECLARE_BLOCKERS`, `RESOLVE_COMBAT` | Assignment order and keyword arithmetic (first/double strike, trample, deathtouch, lifelink, protection, indestructible) are engine-owned (MAD-325). `DECLARE_BLOCKERS` carries both orders: each blocker's list is its order over the attackers it blocks; `attack_orders` carries the attacking player's order over each attacker's blockers (CR 509.3), defaulting to blocker id order when undeclared. |
...
| Declared effects | `DECLARE_EFFECT`, `DECLARE_TRIGGER`, `ORDER_TRIGGERS`, `ADD_MODIFIER`, `REMOVE_MODIFIER` | The scope line's constructive half: known shapes decompose into structured events; the rest lands as `EFFECT_DECLARED` with prose. `DECLARE_TRIGGER` queues a triggered ability (`TRIGGER_FIRED`) — it holds no priority and reaches the stack at the next grant, per APNAP. `ORDER_TRIGGERS` (5c) reorders the waiting queue by fired ordinal; a player may permute only their own entries. |
...
| Group | Kinds | Notes |
|---|---|---|
| Lifecycle | `GAME_STARTED`, `GAME_ENDED` | Config echoed into `GAME_STARTED` so the log is self-describing. `GAME_ENDED` reason `last_standing` is asserted when eliminations leave one player. |
| Turn | `TURN_STARTED` {turn, seat}, `STEP_ENTERED` {phase, step} ★, `TURN_ENDED`, `PLAYER_LEFT` {seat, cause} | `TURN_ENDED` is the expiry anchor for `until_end_of_turn` modifiers and marked damage. Steps include the combat five and cleanup. Untap and cleanup grant no priority (CR 502.3, 514.3a); every other step grants it to the active player as it is entered. `PLAYER_LEFT` causes: `sba_zero_life`, `sba_poison`, `sba_commander_damage` (with the commander's name), `concession` — the leaving seat prunes turn order and takes its owned objects with it (CR 800.4). |
| Priority & stack | `PRIORITY_PASSED`, `STACK_PUSHED` {mode: cast \| activated \| triggered, controller, targets}, `STACK_RESOLVED` | One object per push; the stack is its order in the fold. The pusher retains priority (CR 117.2c); resolution returns it to the active player (CR 117.3b); all living seats passing resolves the top or ends the step (CR 117.4). Casting, activating, playing a land and passing require holding priority — manual bookkeeping never does. A resolving ability applies its declared effect as an `EFFECT_DECLARED` row: the engine owns ordering and timing, never oracle text (ADR 11). |
| Zone moves | `OBJECT_CREATED` {card \| token spec, controller, owner, zone}, `ZONE_CHANGED` {obj, from, to, cause}, `LAND_PLAYED` ★, `CREATURE_ETB` ★, `DIED` ★ {obj, cause}, `CAST` ★ {seat, card, targets} | The starred kinds are the informative ones the log renders by name; generic moves use `ZONE_CHANGED` with a cause (`sacrifice`, `destroy`, `bounce`, `exile`, `mill`, `counter`, …). `DIED` causes: `sba_zero_toughness`, `sba_lethal_damage`, `sba_legend_rule`, `sba_no_loyalty` — engine-asserted so viewers never re-run CR 704; an unattached aura falls via `ZONE_CHANGED` cause `sba_unattached`. |
| Combat & damage | `ATTACKERS_DECLARED` ★ {assignments}, `BLOCKERS_DECLARED` {blockers, attack orders}, `COMBAT_RESOLVED`, `DAMAGE_DEALT` {source, target, amount, combat?}, `DAMAGE_MARKED` {obj, amount, source} | `RESOLVE_COMBAT` turns the declarations into damage rows — two steps when first/double strike is anywhere in combat, deaths asserted between them by the CR 704 sweep — and anchors them with `COMBAT_RESOLVED`, without which the combat damage step cannot be left. Damage on creatures persists until cleanup, hence `DAMAGE_MARKED`; it rides per-source so deathtouch lethality (any nonzero damage from a deathtouch source) is a read, not a guess. Commander damage derives from `DAMAGE_DEALT` rows whose source is a commander — it is never a separate counter to drift. |
...
| Object state | `TAP_CHANGED`, `PHASE_CHANGED`, `ATTACHED`, `UNATTACHED`, `MODIFIER_ADDED`, `MODIFIER_REMOVED`, `OBJECT_CEASED` | Control changes are layer-2 `MODIFIER_ADDED` — there is no `CONTROLLER_CHANGED` kind, on purpose. `OBJECT_CEASED` is the token rule (CR 704.5d): a token off the battlefield ceases to exist. |
| Triggers | `TRIGGER_FIRED` {source, spec}, `TRIGGERS_ORDERED` {order} | `TRIGGER_FIRED` lands in the trigger queue; it reaches the stack as `STACK_PUSHED` (mode `triggered`) at the next priority grant, in APNAP order — the active player's triggers first, then each other player in turn order, each player's own in the order they chose (CR 603.3b). The flush rides the grant and moves priority to no one. `TRIGGERS_ORDERED` (5c) records a controller's chosen order over their own waiting entries, addressed by the `TRIGGER_FIRED` row's ordinal. The registry (5c) fires `TRIGGER_FIRED` rows itself, on the structural kinds, into the same queue. |
