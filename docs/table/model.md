...
| Declared effects | `DECLARE_EFFECT`, `DECLARE_TRIGGER`, `ADD_MODIFIER`, `REMOVE_MODIFIER` | The scope line's constructive half: known shapes decompose into structured events; the rest lands as `EFFECT_DECLARED` with prose. `DECLARE_TRIGGER` queues a triggered ability (`TRIGGER_FIRED`) — it holds no priority and reaches the stack at the next grant, per APNAP. |
...
| Triggers | `TRIGGER_FIRED` {source, spec} | Into the trigger queue; it reaches the stack as `STACK_PUSHED` (mode `triggered`) at the next priority grant, in APNAP order — the active player's triggers first, then each other player in turn order, each player's own in the order they chose (CR 603.3b). The flush rides the grant and moves priority to no one. |
