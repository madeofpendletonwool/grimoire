...
| `POST` | `/api/encounter/design` | `{idea?, party?, difficulty?, objective?, feedback?, current?, notes?, campaign_id?}` | SSE: `meta`, `delta`, `done`, `error` — a whole encounter. Every field is optional; an `objective` kind outside the vocabulary is a 400, never a default. `done` carries `{name, monsters, waves?, unverified, notes, verdict, budget, party, tactics, objective?, ending?, terrain?}` — the tactical read with the model's Tactics prose gated beside it (`prose.prose_status`: `ok` or `rejected` with the figures that did not trace); the last three only when a non-defeat objective was declared |
...
| `POST` | `/api/encounters/tactics` | `{party, monsters, campaign_id?, objective?, terrain?}` | `{tactics:{party_known,caveats,threat,focus,attrition,counterplay,spotlight,movement}}` — the tactical read; every figure carries its `how` derivation. A `campaign_id` fills the party block in (DM scope; a player scope is a 403); without one the read degrades to statblock-only and its caveat says so |
