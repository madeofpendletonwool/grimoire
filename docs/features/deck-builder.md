...
4. **Analyze.** Paste any decklist (counts as `2x Name`, `2 Name`, or `Name x2`; commander line optional). You get color-identity checks, curve, land/ramp/draw/interaction counts, salt warnings, and a model-written critique — grounded in the deterministic analysis.
...
Same as the sage: the model never names a card you haven't seen verified. Commanders are looked up before drafting starts; picks are validated after the draft lands; a pasted list resolves exact-first, then fuzzily, and a name that can't be resolved is reported rather than dropped.
